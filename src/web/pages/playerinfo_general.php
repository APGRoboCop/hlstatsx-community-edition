	...
	<div style="float:left;vertical-align:top;width:48.5%;">
	    ...
	    <tr class="data-table-head">
			...
			<?php
			    ...
			    $xmlDoc = null;
			    if ($xml) {
                                // PHP 8 Fix: Suppress warnings for invalid XML
		...
		<tr class="bg1">
			...
			<?php
			    ...
			    ");
                            // PHP 8 Fix: Replace list()
			    $row = $db->fetch_row();
                            $av_ping = ($row) ? $row[0] : null;
                            $av_latency = ($row) ? $row[1] : null;
		...
		<tr class="bg1">
			...
			<?php
			    ...
			    ");
                            // PHP 8 Fix: Replace list()
                            $row = $db->fetch_row();
                            $favMap = ($row) ? htmlspecialchars($row[0]) : null;
			    if ($favMap)
...
// Current rank & rank history
	...
	$rankHistory .= '<img src="' . $histimage['url'] . '" title="' . htmlspecialchars($result['rankName']) . '" alt="' . htmlspecialchars($result['rankName']) . '" /> ';
    } 
?>
	...
	<div style="float:left;vertical-align:top;width:48.5%;">
		...
		<tr class="bg1">
			...
			<?php echo '<img src="'.$rankimage['url']."\" alt=\"".htmlspecialchars($rankName)."\" title=\"".htmlspecialchars($rankName)."\" />"; ?>
		    </td>
		</tr>
		<tr class="data-table-head">
		    <td style="width:60%;">
...
// Awards
	...
	if(!isset($awards_done[$ribbonCode]))
	{
	    ...
	    elseif ($realgame && file_exists(IMAGE_PATH."/games/$realgame/ribbons/".$result['image']))
	    {
	    ...
	    $ribbonList .= '<img src="'.$image.'" style="border:0px;" alt="'.htmlspecialchars($result['ribbonName']).'" title="'.htmlspecialchars($result["ribbonName"]).'" /> ';
	...
	elseif ($realgame && $image = getImage("/games/$realgame/gawards/".strtolower($a->aType."_$a->code")))
	{
	    $image = $image['url'];
	}
	else
	...
	$GlobalAwardsList .= "<img src=\"$image\" alt=\"".htmlspecialchars($a->ribbonName)."\" title=\"".htmlspecialchars($a->ribbonName)."\" /> ";
    }
