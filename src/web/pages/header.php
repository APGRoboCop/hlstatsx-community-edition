...
*/
	...
	$db->query("UPDATE hlstats_Options SET value=value+1 WHERE keyname='counter_visits';");
        // PHP 8 Fix: Ensure timeout is numeric
        $timeout = isset($g_options['counter_visit_timeout']) ? (int)$g_options['counter_visit_timeout'] : 60;
	@setcookie('ELstatsNEO_Visit', '1', time() + ($timeout * 60), '/');   
    }
    ...
    // PHP 8 Security: Sanitize style input to prevent traversal
    $selectedStyle = basename($selectedStyle);
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="styles/modern-responsive.css?v=1.2" />
    <!-- U R A SMACKHEAD -->
<?php
    if ($mode == 'players')
    {
...
?>
    <title>
<?php
    echo htmlspecialchars($g_options['sitename']); 
    foreach ($title as $t)
    {
...
<?php
    ...
    if (isset($g_options['sourcebans_address']) && !empty($g_options['sourcebans_address']) && file_exists($iconpath . "/title-sourcebans.png")) {
	$extratabs .= "<li><a href=\"". htmlspecialchars($g_options['sourcebans_address']) . "\" target=\"_blank\"><img src=\"" . $iconpath . "/title-sourcebans.png\" alt=\"SourceBans\" /></a></li>\n";
    }
    if (isset($g_options['forum_address']) && !empty($g_options['forum_address']) && file_exists($iconpath . "/title-forum.png")) {
	$extratabs .= "<li><a href=\"" . htmlspecialchars($g_options['forum_address']) . "\" target=\"_blank\"><img src=\"" . $iconpath . "/title-forum.png\" alt=\"Forum\" /></a></li>\n";
    }
?>
<div class="block">
...
<?php
	    if (!empty($g_options['sitename']) && !empty($g_options['siteurl']))
	    {
                // PHP 8 Fix: Ensure string types
                $site_url_clean = preg_replace('/http:\/\//', '', (string)$g_options['siteurl']);
		echo '<li><a href="' . $site_url_clean . '">'. htmlspecialchars($g_options['sitename']) . '</a> <span class="arrow">&raquo;</span></li>';
	    }
	    echo '<li><a href="' . htmlspecialchars($_SERVER['PHP_SELF']) . '">HLstatsX</a>';


	    $i=0;
	    foreach ($location as $l=>$url)
	    {
		$url = preg_replace('/%s/', (string)$g_options['scripturl'], $url);
		$url = preg_replace('/&/', '&amp;', $url);
		echo ' <span class="arrow">&raquo;</span></li><li>';
		if ($url) {
		    echo "<a href=\"$url\">".htmlspecialchars($l)."</a>";
		} else {
		    echo "<strong>".htmlspecialchars($l)."</strong>";
		}
		$i++;
	}
?>			</li>
...
?>
	<div class="fNormal" style="float:right;"> 
		...
		<?php 
			...
			echo "\t\t\t\t<option value=\"".htmlspecialchars($e)."\"$sel>".htmlspecialchars($ename)."</option>\n"; 
		    } ?> 
		</select> 
	    </form> 
	</div> 
...
<?php
    global $mode;
    // PHP 8 Fix: Ensure bannerdisplay key exists
    $banner_display = isset($g_options['bannerdisplay']) ? $g_options['bannerdisplay'] : 0;
    if ($banner_display != 0 && ($mode == 'contents' || $banner_display == 1)) {
        if (!empty($g_options['bannerfile'])) {
?>    
...
<?php
    $result = $db->query("SELECT game from hlstats_Roles WHERE game='$game_esc' AND hidden = '0'");
