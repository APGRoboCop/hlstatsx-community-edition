...
function imagecopymerge_alpha($dst_im, $src_im, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, $pct){
    ...
    // PHP 8 Fix: Cast opacity to int
    imagecopymerge($dst_im, $cut, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, (int)$opacity);
...
function f_num($number) {
	...
	$player_id = valid_request((int)$_GET['player_id'], true);
    } elseif (isset($_GET['steam_id']) && isset($_GET['game'])) {
	$steam_id = valid_request((string)$_GET['steam_id'], false);
...
if ($player_id > 0) {
    ...
    $timestamp   = (int)$playerdata['connection_time'];
    ...
    if (file_exists($trend_image_name)) {
    ...
    if (is_numeric($rank)) {
	imagestring($image, 3, 70, 22, number_format((int)$rank), $font_color);
	$start_pos_x = 71 + (imagefontwidth(3) * strlen(number_format((int)$rank))) + 7;
    } else {
